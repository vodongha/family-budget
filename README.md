...
The app is **self-hosted with Docker on a home server** (not Fly.io); Oracle ADB stays in the cloud.
Work lands on `develop`; merging **`develop → master`** auto-deploys via
[`.github/workflows/deploy.yml`](.github/workflows/deploy.yml): it runs ruff + pytest, then the
`deploy` job POSTs to the home-server webhook (`https://hooks.vodongha.id.vn/deploy/family-budget`,
`Authorization: Bearer ${{ secrets.DEPLOY_TOKEN }}`), which `git pull`s and re-runs
`docker compose up -d --build`. [`sync-develop.yml`](.github/workflows/sync-develop.yml) merges
`master` back into `develop`. The stack is `api` (FastAPI) + `worker` + `beat` (Celery) + local
`redis` + `minio` (see [`docker-compose.yml`](docker-compose.yml) +
[`docker-compose.override.yml`](docker-compose.override.yml) for restart policies). The ADB wallet is
bind-mounted from `./wallet` (gitignored) — never committed.

**Run on the server:**

```bash
# Put the unzipped ADB wallet in ./wallet, fill .env (see .env.example), then:
docker compose up -d --build          # api :8000, worker, beat, redis, minio
```

Config lives in `.env` (git-ignored): `ORACLE_PASSWORD`, `WALLET_PASSWORD`, `ORACLE_DSN`,
**`ORACLE_SCHEMA=FAMILY_BUDGET`** (required — the app's tables live in the `FAMILY_BUDGET` schema, not
the connecting `ADMIN` user's; without it every query fails `ORA-00942`), `JWT_SECRET`,
`ADMIN_SESSION_SECRET`, `REDIS_URL` (local `redis://redis:6379/0`), optional `GOOGLE_CLIENT_ID` /
`GITHUB_TOKEN`. `WALLET_DIR=/app/wallet` matches the bind-mount. Public access
is a **Cloudflare Tunnel** routing `https://famo.io.vn` → `localhost:8000` (no inbound ports; TLS at
Cloudflare's edge). The auto-deploy runs `alembic upgrade head` (a one-off `api` container) before
bringing the stack up; tighten `CORSMiddleware` in `app/main.py` to the web origin for production.

**Bootstrap the first admin** (once):

```bash
docker compose exec api python -m app.scripts.create_admin --email you@example.com --name 'Your Name'
```
