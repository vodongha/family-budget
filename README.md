...
Work lands on `develop`; merging **`develop → master`** auto-deploys to Fly.io via
[`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) (runs ruff + pytest, then
`flyctl deploy`), and [`sync-develop.yml`](.github/workflows/sync-develop.yml) merges `master`
back into `develop`. See [Wiki › Git Workflow](https://github.com/vodongha/family-budget/wiki/Git-Workflow).
The app runs two process groups from one image — `app` (FastAPI, HTTP) and
